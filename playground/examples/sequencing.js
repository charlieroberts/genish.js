...
lookup = phasor( .5 )
freq = peek( data([220,330,440,550]), lookup, 'none' )
...
freq    = peek( data( values ), indexer, 'none', 'samples' )
...
freq = seq( [220,330,440,550], [11025, 22050] )
play( mul( cycle( freq ), .05 ) )

// There's also a third rate parameter (default=1) that lets you modulate 
// timing at audio rate. Here we go from incrementing the
// phase of our sequencer by .5 each sample to 2.5 over 4 seconds

speed = add( .5, mul( phasor( .25 ), 2 ) )
freq = seq( [220,330,440,550], [5512, 11025], speed )
play( mul( cycle( freq ), .05 ) )
