...
function setupMemory( buffer, __pokelength=50 ) {
  utilities.memf   = new Float32Array( buffer )
  utilities.memi   = new Int32Array( buffer )  
...
window.utilities = {
  ...
  clear() {
    //utilities.memf.fill( 0, pokememoryindex, pokememoryindex + pokelength)
    //utilities.memf.fill( 0, memclear )
    utilities.memf.fill(0)
    m = 0
...
// page loads
// XXX this must be added back in to use in node/browser
//fetch( '../dist/main2.wasm')
  //.then( response => response.arrayBuffer() )
  //.then( bytes => wasmbytes = bytes )

// XXX this must also be uncommented to use in node eventually
  ...
  for( let i = 0; i < keys.length; i++ ) {
    ...
    Object.defineProperty( obj, key, {
      ...
      set( v ) {
        ...
        if( isUgen ) {
          utilities.memi[ idx ] = v.idx * 4
        }else{
          utilities.memf[ idx ] = v
        }
      }
    })
    ...
    Object.defineProperty( obj, key, {
      get() {
        const out = statics[ key ].type === 'f'
          ? out = utilities.memf[ idx ]
          : out = utilities.memi[ idx ]
        
        return out
      },
      set(v) {
        if( statics[ key ].type === 'f' )
          utilities.memf[ staticidx++ ] = v
        else
          utilities.memi[ staticidx++ ] = v
      }
    })
    ...
    const obj = {
      ...
      connect( ...ugens ) {
        ugens.forEach( ugen => {          
          ...
          //utilities.memi[ idx   ] = ugen.fid
          utilities.memi[ idx ] = ugen.idx * 4
        
          obj.connected.push( ugen )
        })
        
        utilities.memi[ obj.idx + 2 ] = obj.connected.length
      }
    }
  
    //createProperty( obj, 'gain', obj.idx, gain )

    utilities.memi[ obj.idx + 2 ] = 0
    utilities.memf[ obj.idx + 1 ] = gain
    utilities.memi[ obj.idx ] = fid
  ...
  ssd = function( init=0 ) {
    const obj = {
      ...
      in( input ) {
        ...
        utilities.memi[ obj.idx + 2 ] = memo( input ).idx * 4
      }
    }
    ...
    Object.defineProperty( obj, 'value', {
      ...
      set(v) {
        if( isNaN(v) ) {
          ...
          value = v
          utilities.memf[ obj.idx + 1 ] = value
        }
      }
    })
    
    utilities.memf[ obj.idx + 1 ] = init
    utilities.memi[ obj.idx ] = obj.fid
...
const data = function( __data, type='float' ) {
  ...
  if( typeof __data === 'string' ) { 
    ...
    obj = { 
      ...
      name:'data',
      get( index=0 ) {
        return type === 'float' ?  utilities.memf[ obj.idx + index ] : utilities.memi[ obj.idx + index ]
      },
      set( value, index=0 ) {
        if( type === 'float' ) {
          utilities.memf.set( __data, obj.idx+index )
        }else{
          utilities.memi.set( __data, obj.idx+index )
        }
      },
      peek( index=0 ) {
...
{ 
  ...
  poke = function( data, value=0, index=0 ) {
    ...
    utilities.memi[ pokememoryindex + pokecounter ] = obj.idx * 4
